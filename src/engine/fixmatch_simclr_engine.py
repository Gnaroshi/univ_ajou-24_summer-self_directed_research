...
class FixMatchSimCLREngine(Engine):
    def __init__(self, cfg, fabric, model, model_ema, criterion, optimizer, scheduler, loaders, epochs):
        ...
        self.clvg = sa.ContrastiveLearningViewGenerator(sa.simclr_transform(size=32, device=self.device), n_views=self.cfg.fs.n_views)
    ...
    def info_nce_loss(self, features):
        ...
        # labels = torch.cat([torch.arange(self.cfg.train.batch_size ) for i in range(self.cfg.fs.n_views)], dim=0)
        labels = torch.cat([torch.arange(features.shape[0] // 2) for i in range(self.cfg.fs.n_views)], dim=0)
    ...
    def iterate(self, model, data, criterion):
        ...
        x_sa_i, x_sa_j = self.clvg(x)
        x_sa_i, x_sa_j = map(lambda a: a.to(self.device), [x_sa_i, x_sa_j])
        x_sa = torch.cat([x_sa_i, x_sa_j], dim=0)
        x_sa = x_sa.to(memory_format=torch.channels_last)
        ...
        with self.fabric.autocast():
            ...
            feature = self.model(x_sa)
            sa_logits, sa_labels = self.info_nce_loss(feature)
            Lsa = self.sa_criterion(sa_logits, sa_labels)

            Lx = self.train_criterion(logits_x, y)
            Lu, _ = self.consistency_loss(logits_u_w, logits_u_s)
            loss = Lx + Lu * self.lambda_u + Lsa
